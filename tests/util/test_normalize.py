    ...
    def test__pixel_space(
        ...
        for device in devices:
            ...
            for (x_denorm, _), (x_norm, _) in zip(
                ...
                x_denorm, x_norm = x_denorm.to(device), x_norm.to(device)
                assert normalizer(255.0 * x_denorm).allclose(
                    x_norm, atol=1e-6
...
class TestDenormalizer:
    def test__unit_space(
        ...
        mean, std = cifar10_stats
        for device in devices:
            denormalizer = fourier_attack.util.Denormalizer(
                32, mean, std, device, False
            )
    ...
    def test__pixel_space(
        ...
        for device in devices:
            ...
            for (x_denorm, _), (x_norm, _) in zip(
                ...
                x_denorm, x_norm = x_denorm.to(device), x_norm.to(device)
                assert denormalizer(x_norm).allclose(255.0 * x_denorm, atol=1e-8)
                break  # test only first batch
